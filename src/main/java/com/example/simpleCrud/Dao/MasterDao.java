...
public class MasterDao implements MasterInterface{
    ...
    public List<Master> getAllStudyFields() {
        final String sql = "SELECT * FROM Master ORDER BY ID ASC LIMIT 6";
        try {
    ...
    public void insertStudyField(Master master) {
        final String url = "INSERT INTO Master (technology, industrial, business, political)" +
                " VALUES ('" + master.getTechnology() + "', '" + master.getIndustrial() + "','" + master.getBusiness() + "', '" + master.getPolitical() + "')";
